      ...
      <el-col :span="6" style="margin-top: 16px; padding-left: 12px; padding-right: 12px">
        ...
        >
          <el-form-item label="教师号" prop="tid">
            <el-input clearable v-model.number="form.tid"></el-input>
          </el-form-item>
          <el-form-item label="姓名">
            <el-input clearable v-model="form.name"></el-input>
          </el-form-item>
          <el-form-item label="性别">
            <el-select v-model="form.sex" placeholder="可选择性别">
...
export default {
  data() {
    var checkID = (rule, value, callback) => {
      ...
      else if (String(value).length != 6) return callback(new Error("如要添加数据，ID应为6位"));
      else callback();
    };
    return {
      oldRowJson: null,     
      columns: [
          { field: "tid", title: "教师号", width: 150 },
          { field: "name", title: "姓名", width: 105 },
          { field: "sex", title: "性别", width: 100 },
          { field: "age", title: "年龄", width: 100 },
          { field: "dname", title: "院系名称", width: 250 }, 
      ],
      ploading: false,
      qloading: false,
      currentPage: 1,
  ...
  methods: {
    get() {
      ...
      this.$axios.getTeacher(_this.form).then(
        response => {
          // 成功回调
          ...
          if (res.data.res == true) {
            _this.totalItems = res.data.teacher.length;
            res.data.teacher.map(i => {
                        i.origin=true;
                        return i;
                    });
            _this.tableData = res.data.teacher;
    ...
    post(formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (!valid) return false;
          ...
          _this.ploading = true;
          console.log(_this.form);
          _this.$axios.postTeacher(_this.form).then(
            response => {
              // 成功回调
              ...
              if (res.data.res == true) {
                ...
                this.$message("添加失败，请检查表单，如除了id，必须有一个属性不为空");
              }
            },
            response => {
              // 错误回调
    ...
    handleDelete(index, row) {
      let data = { tablename: "teacher", id: row.tid }
    ...
    handleCheck(index, row) {
      ...
      if(data.tid != oldData.tid)
      {
        data.old_tid = oldData.tid;
      }
      this.$axios.postTeacher(data).then(
        response => {
          // 成功回调
          var res = response.data;
          console.log(res);
